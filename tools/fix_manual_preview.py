...
admin = "const ADMIN=new Set(['hint.kpc@gmail.com','kpc.jtkim@gmail.com']);"
if "const BRIDGE=process.env.ADMIN_DRIVE_REVIEW_BRIDGE||'';" not in s:
    if admin not in s:
        raise SystemExit('ADMIN marker missing')
    s = s.replace(admin, admin + "\nconst BRIDGE=process.env.ADMIN_DRIVE_REVIEW_BRIDGE||'';", 1)

if 'async function callBridge(payload){' not in s:
    marker = 'async function classFields(idToken,classId){'
    if marker not in s:
        raise SystemExit('classFields marker missing')
    bridge = """async function callBridge(payload){
...
if 'const upstreamCount=' not in s:
    old = """  const rawFiles=filesFrom(chosen,folder.df);
...
"""
    new = """  const rawFiles=filesFrom(chosen,folder.df);
  const allFiles=rawFiles.map(safeFile).filter(Boolean);
  const files=allFiles.filter(isPreviewable).sort((a,b)=>uploadedAt(b)-uploadedAt(a));
...
"""
    if old not in s:
        raise SystemExit('monitor count block missing')
    s = s.replace(old, new, 1)

if 'bridgeUsed:true' not in s:
    start_marker = "    const fields=await classFields(idToken,cid),root=coreLink(fields,key);if(!root)throw new Error('FOLDER_NOT_SET');"
    catch_marker = "  }catch(e){return res.status(400).json({ok:false,error:String(e.message||e)})}"
    start = s.find(start_marker)
    end = s.find(catch_marker, start)
    if start < 0 or end < 0:
        raise SystemExit(f'handler boundaries missing start={start} end={end}')
    block = """    const requestedDate=String(date);
...
s = p.read_text(encoding='utf-8')
if '.status.recognized{' not in s:
    old = '.status.absent{background:#fee2e2;color:#991b1b}.status.special{background:#fef3c7;color:#92400e}'
    new = '.status.absent{background:#fee2e2;color:#991b1b}.status.recognized{background:#dbeafe;color:#1d4ed8}.status.special{background:#fef3c7;color:#92400e}'
    if old not in s:
        raise SystemExit('status CSS marker missing')
    s = s.replace(old, new, 1)

old = "function statusClass(s){const x=String(s||'');if(x==='출석')return'present';if(x==='결석')return'absent';if(['지각','조퇴','외출','인정출석','중복'].includes(x))return'special';return''}"
new = "function statusClass(s){const x=String(s||'');if(x==='출석')return'present';if(x==='결석')return'absent';if(x==='인정출석')return'recognized';if(['지각','조퇴','외출','중복'].includes(x))return'special';return''}"
if old in s:
    s = s.replace(old, new, 1)
if "if(x==='인정출석')return'recognized'" not in s:
    raise SystemExit('recognized status class patch failed')

if 'const found=Boolean(d.dateFolderFound||exact);' not in s:
    start_marker = '      const exact=Boolean(d.exactDateFolder&&d.dateFolderId);'
    next_marker = "      if(typeof folderTarget!=='undefined'&&folderTarget){folderTarget.innerHTML=`<strong>${esc(currentIso)} 날짜 폴더 확인됨</strong>"
    start = s.find(start_marker)
    end = s.find(next_marker, start)
    if start < 0 or end < 0:
        raise SystemExit(f'loadDrive boundaries missing start={start} end={end}')
    block = """      const exact=Boolean(d.exactDateFolder&&d.dateFolderId);
      const found=Boolean(d.dateFolderFound||exact);
      ...
      if(!exact){
        const reported=Number.isFinite(Number(d.fileCount))?Number(d.fileCount):0;
...
p = Path('index.html')
s = p.read_text(encoding='utf-8')
s = s.replace('/attendance-overview.js?v=20260906-6', '/attendance-overview.js?v=20260906-7')
p.write_text(s, encoding='utf-8')

print('manual preview + recognized status patch applied')
