...
# replace old checkbox save function wholesale
s=re.sub(
...
# renderDate review state setup and UI
...
new_doc="const docState=docReview.get(s.name)||'미확인';\n    const docHtml=`<div class=\"doc-verify ${status==='인정출석'?'':'hidden'}\" data-doc-wrap=\"${i}\"><label>인정출석 증빙서류 <select class=\"doc-review-select\" data-i=\"${i}\"><option value=\"미확인\" ${docState==='미확인'?'selected':''}>미확인</option><option value=\"확인\" ${docState==='확인'?'selected':''}>확인</option><option value=\"인정불가\" ${docState==='인정불가'?'selected':''}>인정불가</option></select></label><span class=\"doc-save-state\" data-doc-state=\"${i}\">${docState==='미확인'?'선택 시 자동저장':docState+' 저장됨'}</span></div>`;"
if old_doc.search(s): s=old_doc.sub(new_doc,s,count=1)
else:
    # simpler exact fallback
    s=re.sub(r"\s*const docHtml=`<div class=\"doc-verify .*?</div>`;", "\n    "+new_doc, s, count=1, flags=re.S)

# CSS: style select too
...
# 3) Overview: show three-state status, not boolean only.
...
# 4) Save/reset preserve three-state object, while keeping legacy documentVerified for compatibility.
...
# Update reset-enabled test to include documentReview.
...
# Remove duplicate docHtml rendering if present.
