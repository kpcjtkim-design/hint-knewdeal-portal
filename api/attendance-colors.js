...
async function profile(idToken,email){
  ...
  const {response:r,data:d}=await fetchJson(url,{headers:{authorization:`Bearer ${idToken}`}});if(!r.ok&&(r.status===429||d.error?.status==='RESOURCE_EXHAUSTED'))throw new Error('DATABASE_QUOTA_EXCEEDED');
