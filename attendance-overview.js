import {createAttendanceBeta} from './attendance-beta.mjs';
...
const BETA_STYLE=`.workspace{grid-template-columns:280px minmax(0,1fr)}.table-head,.student-row{grid-template-columns:80px 118px 145px 140px 140px 130px 180px 180px 170px 195px 195px;min-width:1773px}.beta-status,.beta-evidence,.beta-reason{width:100%;min-width:0;border:1px solid #cbd5e1;border-radius:8px;padding:8px 5px;background:#fff;font-size:11px}.beta-status:disabled,.beta-evidence:disabled{color:#64748b;background:#f8fafc}.beta-docs{flex-direction:column;justify-content:center;gap:6px}.docmemo{padding:5px 8px;font-size:10px}.beta-source{display:block;font-size:11px;line-height:1.6;white-space:pre-wrap;overflow-wrap:anywhere}.beta-source small{display:block;font-size:9px;color:#64748b}.reason{width:145px}.table-head .btn{font-size:10px;padding:5px;margin-top:5px}.autosave-notice{white-space:normal;font-size:11px}.raw-text{min-height:260px}.rows{max-height:72vh}dialog{max-width:560px;width:95%;border:1px solid #cbd5e1;border-radius:14px;padding:20px}dialog::backdrop{background:#0f172a66}dialog textarea{width:100%;min-height:140px;padding:10px;border:1px solid #cbd5e1;border-radius:8px}dialog .followup-btn{font-size:11px;padding:8px}dialog .btn{margin-top:14px}@media(max-width:1080px){.workspace{grid-template-columns:1fr}}`;
...
export async function mountAttendanceOverview(host,ctx){
  ...
  if(outer){outer.style.overflow='visible';outer.style.width='min(2560px, calc(100vw - 32px))';outer.style.maxWidth='none';outer.style.marginLeft='50%';outer.style.transform='translateX(-50%)'}
  const root=host.shadowRoot||host.attachShadow({mode:'open'});
  root.innerHTML=`<style>${STYLE}${BETA_STYLE}</style><div class="wrap"><div id="err"></div><section class="card toolbar"><div class="field"><label>반</label><select id="classSel">${classes.map(c=>`<option value="${esc(c.id)}">${esc(c.id)}반 · ${esc(c.course||'')}</option>`).join('')}</select></div><div class="field"><label>교육일자</label><select id="dateSel"><option>불러오는 중…</option></select></div><button id="reload" class="btn soft">↻ 다시 읽기</button><button id="sheetConnect" class="btn soft">내 계정 시트 연결</button><button id="excelExport" class="btn dark">⇩ 엑셀 다운로드</button><span class="autosave-notice">※ 출결·서류 상태는 즉시 저장 / 사유는 열 상단 저장 / 관리자 메모는 자동저장</span><div class="spacer"></div><span id="topState" class="state">준비 중…</span></section><div class="hint">베타 · 연결한 개인 Google 계정의 시트 편집 권한으로 선택한 학생·날짜 셀만 저장합니다. 좌측은 가-3 원문입니다.</div><section class="workspace"><aside class="left-stack"><article class="card raw-card"><div class="section-head"><div><h3>가-3 원문</h3><p>선택한 교육일의 Google Sheet 원문 내용입니다. · 가공하지 않은 원문 텍스트</p></div></div><pre id="rawReason" class="raw-text">Google Sheet를 불러오는 중…</pre></article><article class="card manual-card"><div class="section-head"><div><h3>수기출석 관련 관리자 메모</h3><p>해당 반·날짜 수기출석 전체에 대한 관리자 메모입니다.</p></div></div><div class="manual-body"><div class="manual-issue"><div class="manual-issue-head"><strong>관리자 메모</strong><span id="manualIssueState" class="manual-issue-state"></span></div><textarea id="manualIssueMemo" placeholder="전반적인 출결 특이사항, 전달사항 등을 입력하세요."></textarea><div class="followup-wrap general-followup"><div class="followup-buttons"><button type="button" id="manualNotifyBtn" class="followup-btn notified">담임 알림</button><button type="button" id="manualDoneBtn" class="followup-btn done">이행 확인</button></div></div></div></div></article></aside><article class="card data-panel"><div class="data-title"><strong>Google Sheet · 출결현황 + 학생별 관리자 메모</strong><span>시트 출결 · 서류 상태 · 체크히어 저장본 · 관리자 메모</span></div><div class="table-scroll"><div class="table-head"><div>이름</div><div>출석현황</div><div>사유 <button id="saveReasons" class="btn soft" disabled>사유 저장</button></div><div>서류제출 · 메모</div><div>입실 → 퇴실</div><div>시간 판정</div><div>입실·교시 관리자 메모</div><div>퇴실 관리자 메모</div><div>외출 구간</div><div>체크히어 관련 메모</div><div>수기출석 관련 메모</div></div><div id="rows" class="rows"><div class="empty">불러오는 중…</div></div></div></article></section><dialog id="documentMemoDialog"></dialog></div>`;
  ...
  const beta=createAttendanceBeta({db,user,root,state:()=>({classId:currentClass,iso:currentIso,date:dates.find(x=>x.label===dateSel.value),students,raw:String(reasonCells[dateSel.value]||''),backgrounds:attendanceBackgrounds,setRaw(value){reasonCells[dateSel.value]=value;renderRawReason(dateSel.value);}}),render:()=>renderRows(dateSel.value),showErr:e=>showErr(e),reasonFor,colorState:overviewColorState});
...
async function getColors(cid){
  ...
  async function saveStudentMemo(key,category,value,stateEl,cid,iso){
    ...
    try{await setDoc(doc(db,'settings',memoId(cid,iso)),{type:'ATTENDANCE_OVERVIEW_MEMO',classId:cid,date:iso,memos:{[key]:{[category]:String(value||'')}},updatedBy:user.email||'',updatedAt:serverTimestamp()},{merge:true});if(cid===currentClass&&iso===currentIso&&stateEl)stateEl.textContent='저장됨'}catch(e){if(stateEl)stateEl.textContent='실패';throw e}
  }
  ...
  async function saveStudentFollowup(key,category,field){
    ...
    await setDoc(doc(db,'settings',memoId(currentClass,currentIso)),{type:'ATTENDANCE_OVERVIEW_MEMO',classId:currentClass,date:currentIso,memos:{[key]:{followup:{[category]:f}}},updatedBy:user.email||'',updatedAt:serverTimestamp()},{merge:true});return true
  }
  ...
  function renderRows(label){
    ...
    rows.innerHTML=students.map((s,i)=>{const key=keyFor(s),bundle=normalizeMemoBundle(memos[key]),controls=beta.controls(s);return`<div class="student-row"><div class="cell"><div class="name">${esc(s.name)}</div></div><div class="cell center">${controls.status}</div><div class="cell reason">${controls.reason}</div><div class="cell beta-docs">${controls.evidence}<button class="btn ghost docmemo" data-docmemo="${esc(key)}">${bundle.documents?'● 메모 있음':'＋ 메모'}</button></div>${beta.snapshotCells(s)}<div class="cell memo-cell"><textarea class="memo" data-key="${esc(key)}" data-category="checkhere" data-state-key="${i}-checkhere" placeholder="체크히어 관련 메모">${esc(bundle.checkhere)}</textarea><span class="memo-state" data-state="${i}-checkhere">${bundle.checkhere?'저장됨':''}</span>${followupControls(key,'checkhere',bundle.followup?.checkhere)}</div><div class="cell memo-cell"><textarea class="memo" data-key="${esc(key)}" data-category="manual" data-state-key="${i}-manual" placeholder="수기출석 관련 메모">${esc(bundle.manual)}</textarea><span class="memo-state" data-state="${i}-manual">${bundle.manual?'저장됨':''}</span>${followupControls(key,'manual',bundle.followup?.manual)}</div></div>`}).join('')||'<div class="empty">교육생이 없습니다.</div>';
    beta.bind();root.querySelectorAll('[data-docmemo]').forEach(button=>button.onclick=()=>openDocumentMemo(button.dataset.docmemo));
  ...
  function openDocumentMemo(key){
    ...
    dialog.innerHTML=`<h3>${esc(s?.name||'')} · 서류제출 관련 메모</h3><textarea id="docMemoText" aria-label="서류제출 관련 메모">${esc(bundle.documents)}</textarea><span id="docMemoState" class="memo-state"></span>${followupControls(key,'documents',bundle.followup.documents)}<button id="docMemoClose" class="btn dark">닫기</button>`;
    const input=dialog.querySelector('#docMemoText'),status=dialog.querySelector('#docMemoState');let timer,pending=Promise.resolve();
    const save=()=>{clearTimeout(timer);pending=saveStudentMemo(key,'documents',input.value,status,cid,iso).catch(e=>{showErr(e);throw e;});pending.catch(()=>{});return pending;};
    input.oninput=()=>{status.textContent='입력 중';clearTimeout(timer);timer=setTimeout(save,650);};
    const close=async()=>{try{await save();dialog.close();renderRows(dateSel.value);}catch{status.textContent='저장 실패';}};
    dialog.querySelector('#docMemoClose').onclick=close;dialog.oncancel=e=>{e.preventDefault();close();};
    dialog.querySelectorAll('[data-followup-field]').forEach(b=>b.onclick=async()=>{try{await save();await saveStudentFollowup(key,'documents',b.dataset.followupField);dialog.close();renderRows(dateSel.value);openDocumentMemo(key);}catch(e){showErr(e);}});
    dialog.showModal();
  }
  ...
  async function exportCurrentExcel(){
    ...
    try{
      ...
      const aoa=[
        ...
        [],
        ['이름','출석현황','사유','서류제출','체크히어 관련 메모','체크히어-담임 알림','체크히어-이행 확인','서류제출 관련 메모','서류제출-담임 알림','서류제출-이행 확인','수기출석 관련 메모','수기출석-담임 알림','수기출석-이행 확인','체크히어 입실','체크히어 퇴실','체크히어 입실 메모','체크히어 퇴실 메모','체크히어 외출 구간','체크히어 수집 시각']
      ];
      for(const st of students){
        ...
        const bundle=normalizeMemoBundle(memos[key]);aoa.push([st.name,status,reason||'-',e.label,memoValueNow(key,'checkhere'),followupExport(bundle.followup.checkhere.notifiedAt),followupExport(bundle.followup.checkhere.doneAt),memoValueNow(key,'documents'),followupExport(bundle.followup.documents.notifiedAt),followupExport(bundle.followup.documents.doneAt),memoValueNow(key,'manual'),followupExport(bundle.followup.manual.notifiedAt),followupExport(bundle.followup.manual.doneAt),ch?.rawEntry??ch?.entry??'',ch?.exit||'',ch?.entryMemo||'',ch?.exitMemo||'',ch?.outings?.map(x=>x.start+'~'+x.end).join(', ')||'',ch?.collectedAt||'']);
      }
  ...
  async function loadSelectedDate(){showErr('');const label=dateSel.value,d=dates.find(x=>x.label===label);if(!d)return;currentIso=d.iso;topState.textContent=`${currentClass}반 · ${label} 불러오는 중…`;await Promise.all([loadMemos(currentClass,currentIso),beta.load(currentClass,currentIso)]);renderRawReason(label);renderRows(label);topState.textContent=`${currentClass}반 · ${label} · ${students.length}명`}
  ...
  classSel.onchange=()=>{if(beta.canNavigate())loadClass(classSel.value);else classSel.value=currentClass;};
