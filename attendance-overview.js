...
export async function mountAttendanceOverview(host,ctx){
  ...
  root.innerHTML=`<style>${STYLE}</style><div class="wrap"><div id="err"></div><section class="card toolbar"><div class="field"><label>반</label><select id="classSel">${classes.map(c=>`<option value="${esc(c.id)}">${esc(c.id)}반 · ${esc(c.course||'')}</option>`).join('')}</select></div><div class="field"><label>교육일자</label><select id="dateSel"><option>불러오는 중…</option></select></div><button id="reload" class="btn soft">↻ 다시 읽기</button><span class="autosave-notice">※ 모든 메모는 저장버튼 없이 자동저장됩니다.</span><div class="spacer"></div><span id="topState" class="state">준비 중…</span></section><div class="hint">원본 Google Sheet와 Google Drive는 읽기만 합니다. 서류제출 상태는 출결 자동검증과 동일한 시트 색상 기준으로 표시합니다.</div><section class="workspace"><article id="previewPanel" class="card preview-panel"><div class="panel-head"><div><h3>수기출석부 미리보기</h3><span id="fileCount" class="count">0개</span></div><div class="panel-actions"><select id="fileSel" class="file-select"></select><button id="openFile" class="btn ghost" disabled>새 창</button><button id="full" class="btn dark">크게 보기</button><span class="size-row">높이 <input id="height" type="range" min="480" max="950" step="10" value="700"></span></div></div><div id="viewer" class="viewer-wrap"><div class="viewer-empty">반과 날짜를 불러오는 중입니다.</div></div><div class="manual-issue"><div class="manual-issue-head"><strong>수기출석부 이슈 메모</strong><span id="manualIssueState" class="manual-issue-state"></span></div><textarea id="manualIssueMemo" placeholder="시스템오류 누락, 관리자 서명 누락 등 기재"></textarea></div></article><article class="card data-panel"><div class="data-head"><div>Google Sheet · 이름 / 출석현황 / 가-3 사유 / 서류제출</div><div>학생별 관리자 메모 · 체크히어 / 서류제출 / 수기출석</div></div><div id="rows" class="rows"><div class="empty">불러오는 중…</div></div></article></section></div>`;
  const $=s=>root.querySelector(s),classSel=$('#classSel'),dateSel=$('#dateSel'),rows=$('#rows'),err=$('#err'),topState=$('#topState'),viewer=$('#viewer'),fileSel=$('#fileSel'),openFile=$('#openFile'),fileCount=$('#fileCount'),previewPanel=$('#previewPanel'),manualIssueMemo=$('#manualIssueMemo'),manualIssueState=$('#manualIssueState');
