...
import {createAttendanceBeta} from './attendance-beta.mjs?v=20260914-live2';
...
async function getColors(cid){const bg=await getColorsOnce(cid);if(!Array.isArray(bg)||!bg.length)throw Error('색상 데이터가 비어 있습니다.');return bg;}
  ...
  function renderRows(label){
    ...
    rows.innerHTML=students.map((s,i)=>{const key=keyFor(s),bundle=normalizeMemoBundle(memos[key]),controls=beta.controls(s);return`<div class="student-row"><div class="cell"><div class="name">${esc(s.name)}</div></div><div class="cell center">${controls.status}</div><div class="cell reason">${controls.reason}</div><div class="cell beta-docs">${controls.evidence}<button class="btn ghost docmemo" data-docmemo="${esc(key)}">${bundle.documents?'● 기타 특이사항 있음':'＋ 기타 특이사항'}</button></div>${beta.snapshotCells(s,category=>columnMemoButton(s,category))}<div class="cell memo-cell"><textarea class="memo" data-key="${esc(key)}" data-category="manual" data-state-key="${i}-manual" placeholder="수기출석 관련 메모">${esc(bundle.manual)}</textarea><span class="memo-state" data-state="${i}-manual">${bundle.manual?'저장됨':''}</span>${followupControls(key,'manual',bundle.followup?.manual)}</div></div>`}).join('')||'<div class="empty">교육생이 없습니다.</div>';
