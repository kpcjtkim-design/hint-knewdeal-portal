...
async function getColors(cid){
  ...
  function renderRows(label){
    ...
    rows.innerHTML=students.map((s,i)=>{const key=keyFor(s),bundle=normalizeMemoBundle(memos[key]),controls=beta.controls(s);return`<div class="student-row"><div class="cell"><div class="name">${esc(s.name)}</div></div><div class="cell center">${controls.status}</div><div class="cell reason">${controls.reason}</div><div class="cell beta-docs">${controls.evidence}<button class="btn ghost docmemo" data-docmemo="${esc(key)}">${bundle.documents?'● 메모 있음':'＋ 메모'}</button></div>${beta.snapshotCells(s,category=>columnMemoButton(s,category))}<div class="cell memo-cell"><textarea class="memo" data-key="${esc(key)}" data-category="manual" data-state-key="${i}-manual" placeholder="수기출석 관련 메모">${esc(bundle.manual)}</textarea><span class="memo-state" data-state="${i}-manual">${bundle.manual?'저장됨':''}</span>${followupControls(key,'manual',bundle.followup?.manual)}</div></div>`}).join('')||'<div class="empty">교육생이 없습니다.</div>';
  ...
  function columnMemoButton(s,category){
    ...
    return `<button type="button" class="btn ghost docmemo" data-column-memo="${category}" data-memo-key="${esc(key)}" aria-label="${esc(s.name+' '+MEMO_LABELS[category])}">${exists?'● 메모 있음':'＋ 메모'}</button>`;
  }
  function openStudentMemo(key,category){
    ...
    const legacy=category==='checkhereTimes'&&(bundle.checkhere||followupHasAny(bundle.followup.checkhere));
    dialog.innerHTML=`<h3>${esc(s?.name||'')} · ${esc(MEMO_LABELS[category])}</h3><textarea id="docMemoText" aria-label="${esc(MEMO_LABELS[category])}">${esc(bundle[category])}</textarea><span id="docMemoState" class="memo-state"></span>${followupControls(key,category,bundle.followup[category])}${legacy?'<div class="legacy-memo"><button id="legacyMemo" class="btn soft">기존 통합 체크히어 메모 보기</button></div>':''}<button id="docMemoClose" class="btn dark">닫기</button>`;
  ...
  async function exportCurrentExcel(){
    ...
    try{
      ...
      const aoa=[
        ...
        [],
        ['이름','출석현황','사유','서류제출','체크히어 관련 메모','체크히어-담임 알림','체크히어-이행 확인','서류제출 관련 메모','서류제출-담임 알림','서류제출-이행 확인','수기출석 관련 메모','수기출석-담임 알림','수기출석-이행 확인','체크히어 입실','체크히어 퇴실','체크히어 입실 메모','체크히어 퇴실 메모','체크히어 외출 구간','체크히어 수집 시각',...CHECKHERE_MEMO_CATEGORIES.flatMap(c=>[MEMO_LABELS[c],MEMO_LABELS[c]+'-담임 알림',MEMO_LABELS[c]+'-이행 확인'])]
      ];
      for(const st of students){
        ...
        const bundle=normalizeMemoBundle(memos[key]);aoa.push([st.name,status,reason||'-',e.label,memoValueNow(key,'checkhere'),followupExport(bundle.followup.checkhere.notifiedAt),followupExport(bundle.followup.checkhere.doneAt),memoValueNow(key,'documents'),followupExport(bundle.followup.documents.notifiedAt),followupExport(bundle.followup.documents.doneAt),memoValueNow(key,'manual'),followupExport(bundle.followup.manual.notifiedAt),followupExport(bundle.followup.manual.doneAt),ch?.rawEntry??ch?.entry??'',ch?.exit||'',ch?.entryMemo||'',ch?.exitMemo||'',ch?.outings?.map(x=>x.start+'~'+x.end).join(', ')||'',ch?.collectedAt||'',...CHECKHERE_MEMO_CATEGORIES.flatMap(c=>[memoValueNow(key,c),followupExport(bundle.followup[c].notifiedAt),followupExport(bundle.followup[c].doneAt)])]);
      }
