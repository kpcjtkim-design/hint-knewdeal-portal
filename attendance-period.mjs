...
export async function mountAttendancePeriod(host,{db,user,classes}){
 const root=host.shadowRoot||host.attachShadow({mode:'open'});let busy=false,prepared=[],files=[],checks=[],failures=[],disposed=false,templates=null,selection=null;
 ...
 const invalidate=()=>{prepared=[];files=[];checks=[];failures=[];selection=null;$('#periodGenerate').disabled=true;$('#periodDownloads').innerHTML='';$('#periodResults').textContent='';};
 const lock=value=>{busy=value;root.querySelectorAll('input,select,#periodCheck,#periodGenerate,#periodDownloads button').forEach(e=>e.disabled=value);$('#periodGenerate').disabled=value||!prepared.length;$('#periodCancel').disabled=!value;};
 ...
 const checkedDownload=async(ids,perform,includeFailures=false)=>{
  if(busy||disposed)return;operation=new AbortController();lock(true);
  try{const summary=attendanceDownloadPreflight(checks.filter(c=>ids.includes(c.classId)),{label:`${selection.from} ~ ${selection.to} · ${ids.map(id=>id+'반').join(', ')}`,failures:includeFailures?failures:[]});
   if(await confirmDownloadPreflight(summary,{signal:operation.signal})){operation.signal.throwIfAborted();if(!disposed)await perform();}
   else write('다운로드를 보류했습니다. 점검 결과를 확인한 뒤 다시 다운로드할 수 있습니다.');
  }catch(e){write('다운로드 중단 · '+e.message);}finally{if(!disposed)lock(false);}
 };
 $('#periodGenerate').onclick=async()=>{
  ...
  try{
   ...
   operation.signal.throwIfAborted();if(disposed)return;const zipButton=document.createElement('button');zipButton.textContent=`선택 ${files.length}개 반 ZIP 다운로드`;zipButton.onclick=()=>checkedDownload(files.map(f=>f.id),async()=>{const zip=new window.JSZip();files.forEach(f=>zip.file(f.name,f.bytes));const bytes=await zip.generateAsync({type:'uint8array',compression:'DEFLATE'});if(!disposed&&!operation.signal.aborted)download(bytes,`${files.length===17?'1-17':files.map(f=>f.id).join(',')}반_${selection.period}단위출석부_${selection.from.slice(5).replace('-','')}-${selection.to.slice(5).replace('-','')}.zip`,'application/zip');},true);$('#periodDownloads').append(zipButton);
   for(const file of files){const b=document.createElement('button');b.textContent=file.id+'반 Excel 다운로드';b.onclick=()=>checkedDownload([file.id],()=>download(file.bytes,file.name,'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'));$('#periodDownloads').append(b);}
