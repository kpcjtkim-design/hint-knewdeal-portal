...
function readerFailure(error){
  ...
  return{status:code==='READER_TIMEOUT'?504:503,error:code,...(typeof error.retryable==='boolean'?{retryable:error.retryable}:{}),message:code==='READER_INVALID_DATA'?'시트 응답 형식 또는 반 연결을 확인해야 합니다. 다시 읽기로 재확인해 주세요.':code==='READER_TIMEOUT'?'Google 시트 응답이 늦어 이번 조회를 중단했습니다. 잠시 후 다시 읽어 주세요.':'Google 시트 연결이 일시적으로 불안정합니다. 잠시 후 다시 읽어 주세요.'};
}
