...
async function readerFor(slot,token){
 ...
 const reader=createSurveyReader(async()=>token);await reader.connect();reader.beginResponseRun();readers.set(key,{reader,at:Date.now()});return reader;
}
export async function syncClassWorker({classId,slot,token,idToken,store=createSyncStore(idToken),readSheet,reader,catalog,now=new Date()}){
 ...
 const derived=deriveAttendanceClass(sheet,{classId:cid,metadata,records,entries,identities,today});derived.fingerprint=derivedFingerprint(derived);
