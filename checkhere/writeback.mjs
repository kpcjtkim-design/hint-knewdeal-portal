...
import {requestMatchesRecord,prepareApproval,cleanRequest} from './approval-core.mjs';
...
export async function applyVerified(adapter,record,input,journal,request=null,identities=[]){
  ...
  const next=request?validateProposal(current,{...input,...prepareApproval(request,current,{identities}).after,version:current.version},{requested:true}):validateProposal(record,input);
  ...
  for(const [field,timeKey,memoKey] of [['entry','entry','entryMemo'],['exit','exit','exitMemo']]){
    ...
    let error=null;
    try{const written=await adapter.write(current,field,{time:next[timeKey],memo:next[memoKey]},requested?{requestedFields:Object.keys(requested)}:undefined);if(written){next[timeKey]=written.time;next[memoKey]=written.memo;}}catch(e){error=e.message;}
    let fresh;
    ...
    const matched=fresh.source==='live'&&fresh.readState==='complete'&&(requested?[timeKey,memoKey].filter(k=>Object.hasOwn(requested,k)).every(k=>fresh[k]===requested[k]):fresh[timeKey]===next[timeKey]&&fresh[memoKey]===next[memoKey]);
