...
export function confirmAlreadyApplied(request,record,current){
  if(!current||current.id!==record.id||current.studentKey!==record.studentKey||!record.studentKey||!requestMatchesRecord(request,current))return false;
  return true;
}
...
export async function applyVerified(adapter,record,input,journal,request=null){
  const results=[];
  ...
  if(request&&confirmAlreadyApplied(request,record,current))return{status:'verified',alreadyApplied:true,message:'이미 반영 · 체크히어 원본에서 요청값을 확인했습니다.',results,current};
