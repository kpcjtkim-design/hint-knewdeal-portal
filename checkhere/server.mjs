  ...
  const server=http.createServer(async(req,res)=>{
    try{
      ...
      if(url.pathname==='/api/session'&&req.method==='GET'){
        if(!localOrigin(origin))return send(res,403,{error:'연결 키는 이 PC 화면에서 확인해 주세요.'});return send(res,200,{key,local:true,build:'20260917.2',capabilities:['approved-requests-v1','memo-only-requests-v1','approval-current-sync-v1'],pid:process.pid});
      }
      if(url.pathname.startsWith('/api/')){
        ...
        if(req.method==='GET'&&url.pathname==='/api/state')return send(res,200,{rules:RULES,capabilities:['approved-requests-v1','memo-only-requests-v1','approval-current-sync-v1'],busy,jobs:[...jobs.values()].slice(-20).reverse(),connected:await adapter.loggedIn(),records:latest().map(r=>({...r,audit:judge(r)}))});
