...
test('beta UI separates initial entry, corrections, reason save and evidence confirmation',async()=>{
  ...
  try{
    await page.route('**/*',async route=>{
      ...
      const path=url.pathname.slice(1);if(['attendance-overview.js','attendance-beta.mjs','attendance-beta-core.mjs','attendance-beta-sheet.mjs','checkhere-snapshots.mjs','checkhere/rules.mjs','checkhere-proposals.mjs','checkhere-proposal-core.mjs','checkhere/approval-core.mjs'].includes(path))return route.fulfill({contentType:'text/javascript',body:readFileSync(join(base,path),'utf8')});
      return route.abort();
    });
    ...
    await page.locator('[data-docmemo]').first().click();await page.getByLabel('서류제출 관련 메모',{exact:true}).fill('서류 추가 확인');await page.getByRole('button',{name:'닫기',exact:true}).click();await page.locator('[data-docmemo]').first().getByText('● 내부 특이사항 있음',{exact:true}).waitFor();
    ...
    for(const [category,label] of Object.entries(categories)){
      ...
      await page.getByRole('button',{name:'닫기',exact:true}).click();await page.getByRole('button',{name:'가상가 '+label,exact:true}).getByText('● 내부 특이사항 있음').waitFor();
    }
