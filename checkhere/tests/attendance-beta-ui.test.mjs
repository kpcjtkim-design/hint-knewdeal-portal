...
test('beta UI separates initial entry, corrections, reason save and evidence confirmation',async()=>{
  ...
  const names=['가상가','가상나'],statuses=['해당없음','출석'],colors=['#ffffff','#ffffff'],writes=[];let raw='가상나: 시험',dialogs=[],answer=true,readFailure=false;
  ...
  try{
    await page.route('**/*',async route=>{
      ...
      const path=url.pathname.slice(1);if(['attendance-derived-core.mjs', 'attendance-derived-store.mjs', 'survey-scores.mjs', 'survey-statistics.mjs', 'attendance-statistics.mjs', 'metrics-ui.mjs', 'metrics.css', 'lecture-materials.mjs', 'checkhere/rules.mjs','survey-links.mjs','survey-core.mjs','survey-catalog.json','timetable-holiday.mjs','survey-view.mjs','survey-store.mjs','survey-google.mjs','survey.css','attendance-beta-core.mjs','attendance-rollout.mjs','attendance-io.mjs','attendance-io.mjs','attendance-overview.js','attendance-beta.mjs','attendance-beta-core.mjs','attendance-beta-sheet.mjs','checkhere-snapshots.mjs','checkhere/rules.mjs','checkhere-request-actions.mjs','timetable-core.mjs','checkhere-proposals.mjs','attendance-reason-parser.mjs','checkhere-proposal-core.mjs','checkhere/approval-core.mjs'].includes(path))return route.fulfill({contentType:path.endsWith('.json')?'application/json':'text/javascript',body:readFileSync(join(base,path),'utf8')});
      return route.abort();
    });
