...
test('timetable imports, edits, conflict-checks, publishes and renders teacher mobile view',async()=>{
 ...
 await page.route('**/*',async route=>{const u=new URL(route.request().url());
  ...
  const name=u.pathname.slice(1);if(['survey-links.mjs','survey-core.mjs','survey-catalog.json','timetable-holiday.mjs','survey-view.mjs','survey-store.mjs','survey-google.mjs','survey.css','attendance-beta-core.mjs','attendance-rollout.mjs','attendance-io.mjs','timetable.mjs','timetable-board.mjs','timetable-store.mjs','timetable-core.mjs','timetable.css','timetable-seed.json','timetable-reference.mjs','timetable-reference.json'].includes(name))return route.fulfill({contentType:name.endsWith('.css')?'text/css':name.endsWith('.json')?'application/json':'text/javascript',body:readFileSync(join(base,name),'utf8')});return route.abort();
 });
 ...
 await first.dragTo(second);await page.getByRole('status').getByText(/수업 날짜를 맞바꿨습니다/).waitFor();
