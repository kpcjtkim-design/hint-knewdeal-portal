...
test('administrators submit explicit targets independently of Sheet and collector diagnostics',async()=>{
 ...
 try{
  await page.route('**/*',async route=>{
   ...
   const p=u.pathname.slice(1);if(['checkhere-name-core.mjs','checkhere-name-store.mjs','survey-identity.mjs','attendance-derived-core.mjs','survey-links.mjs','survey-core.mjs','survey-catalog.json','timetable-holiday.mjs','survey-view.mjs','survey-store.mjs','survey-google.mjs','survey.css','attendance-beta-core.mjs','attendance-rollout.mjs','attendance-io.mjs','attendance-io.mjs','checkhere-request-actions.mjs','timetable-core.mjs','checkhere-proposals.mjs','attendance-reason-parser.mjs','checkhere-proposal-core.mjs','attendance-beta-core.mjs','checkhere/approval-core.mjs','checkhere/rules.mjs'].includes(p))return route.fulfill({contentType:'text/javascript',body:readFileSync(join(base,p),'utf8')});return route.abort();
  });
