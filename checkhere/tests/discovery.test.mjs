...
test('delayed lecture rows and paginated dates load before searching',async()=>{
  ...
  let homeLoads=0,lectureLoads=0;
  try{
    await page.route('https://check.ihereapp.com/**',async route=>{
      ...
      if(u.pathname==='/history/lecture'){homeLoads++;html+=`<table><tbody id="rows"></tbody></table><script>setTimeout(()=>{document.querySelector('#rows').innerHTML='<tr><td>[3반] 가상 강의</td><td><button id="view">보기</button></td></tr>';document.querySelector('#view').onclick=()=>location.href='/history/lecture/fixture?academyId=fixture';},350);</script>`;}
      ...
      if(u.pathname.endsWith('/fixture'))lectureLoads++;
    ...
    const result=await c.discover('3','2026-09-07');assert.equal(new URL(result).searchParams.get('scheduleDate'),'2026-09-07');
    // A second date in the same class skips the homepage/class lookup.
    const second=await c.discover('3','2026-09-03');assert.equal(new URL(second).searchParams.get('scheduleDate'),'2026-09-03');assert.equal(homeLoads,1);assert.equal(lectureLoads,2);
