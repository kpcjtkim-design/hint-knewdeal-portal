...
test('identity editor saves only duplicate suffixes into existing class settings and rejects full phone numbers',async()=>{
 ...
 await page.route('**/*',async route=>{const u=new URL(route.request().url());
  ...
  const name=u.pathname.slice(1);if(['survey-identity.mjs','survey-identity-editor.mjs','survey-store.mjs','survey-core.mjs','survey-sync-core.mjs','attendance-io.mjs','attendance-beta-core.mjs'].includes(name))return route.fulfill({contentType:'text/javascript',body:readFileSync(join(base,name),'utf8')});return route.abort();
 });
