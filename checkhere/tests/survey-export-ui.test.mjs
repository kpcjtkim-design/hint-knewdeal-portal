...
test('admin downloads fresh raw without Firebase writes; partial failure requires explicit download; teacher cannot export',async()=>{
 ...
 try{
  ...
  const run=async()=>{await page.getByRole('button',{name:'최신 RAW 한판 다운로드',exact:true}).click();await page.getByRole('dialog').waitFor({state:'visible'});assert.equal(await page.getByRole('button',{name:'엑셀 다운로드',exact:true}).count(),0);await page.screenshot({path:join(base,'checkhere/test-results/survey-raw-export.png')});await page.getByRole('button',{name:'최신 원본으로 다운로드',exact:true}).click();};
  await run();await page.waitForFunction(()=>window.downloads.length===1);let file=await page.evaluate(()=>window.downloads[0]);assert.equal(file.book.sheets[0].sheet.rows[5][20],10);assert.equal(file.book.sheets[0].sheet.rows[5][7],'');assert.equal(file.book.sheets[0].sheet.rows[5][21],'=원본 문장');assert(!file.name.includes('부분자료'));assert.equal(await page.evaluate(()=>window.writes),0);
  await page.getByRole('button',{name:'닫기',exact:true}).click();value=7;await run();await page.waitForFunction(()=>window.downloads.length===2);file=await page.evaluate(()=>window.downloads[1]);assert.equal(file.book.sheets[0].sheet.rows[5][20],7);assert.equal(googleReads,4);
  await page.getByRole('button',{name:'닫기',exact:true}).click();fail=true;await run();await page.getByRole('button',{name:'부분 자료 다운로드',exact:true}).waitFor();assert.equal(await page.evaluate(()=>window.downloads.length),2);await page.getByRole('button',{name:'부분 자료 다운로드',exact:true}).click();await page.waitForFunction(()=>window.downloads.length===3);assert.match((await page.evaluate(()=>window.downloads[2])).name,/_부분자료/);assert.equal(await page.evaluate(()=>window.writes),0);
  await page.getByRole('button',{name:'닫기',exact:true}).click();await page.evaluate(()=>window.mount(true));await page.getByRole('heading',{name:'우리 반 만족도조사'}).waitFor();assert.equal(await page.getByRole('button',{name:'최신 RAW 한판 다운로드',exact:true}).count(),0);assert.deepEqual(errors,[]);
