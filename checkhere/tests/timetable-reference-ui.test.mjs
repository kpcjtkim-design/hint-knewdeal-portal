...
test('review applies only metadata atomically and rejects a stale migration without partial writes',async()=>{
 ...
 try{
  await page.route('**/*',async route=>{const u=new URL(route.request().url());
   ...
   const name=u.pathname.slice(1);if(/^timetable[-.][a-z.-]+$/.test(name)||['survey-links.mjs','survey-core.mjs','survey-catalog.json','attendance-beta-core.mjs'].includes(name))return route.fulfill({contentType:name.endsWith('.css')?'text/css':name.endsWith('.json')?'application/json':'text/javascript',body:readFileSync(join(base,name),'utf8')});return route.abort();
  });
