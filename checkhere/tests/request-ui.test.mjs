  ...
  try{
    for(const role of ['teacher','staff','approver']){
      ...
      await page.route('**/*',async route=>{
        ...
        if(url.hostname==='www.gstatic.com')return route.fulfill({contentType:'text/javascript',body:`export const collection=(...x)=>x,doc=(...x)=>({id:x.at(-1)}),query=(...x)=>x,where=(...x)=>x,orderBy=(...x)=>x,limit=x=>x,serverTimestamp=()=>({seconds:1});export async function setDoc(ref,data){window.rows.push({id:ref.id,...data});}export async function getDocs(){return{docs:window.rows.map(r=>({id:r.id,data:()=>r}))};}export async function runTransaction(db,fn){return fn({get:async ref=>({data:()=>window.rows.find(r=>r.id===ref.id)}),update(ref,data){Object.assign(window.rows.find(r=>r.id===ref.id),data);}});}`});
        ...
        if(['checkhere-requests.mjs','checkhere/approval-core.mjs','checkhere/rules.mjs'].includes(url.pathname.slice(1)))return route.fulfill({contentType:'text/javascript',body:readFileSync(join(base,url.pathname.slice(1)),'utf8')});
        return route.abort();
      });
