  ...
  try{
    for(const role of ['teacher','staff','approver']){
      ...
      await page.route('**/*',async route=>{
        ...
        if(url.pathname==='/')return route.fulfill({contentType:'text/html; charset=utf-8',body:`<div id="host"></div><script type="module">import {mountCheckHereRequests} from '/checkhere-requests.mjs';window.rows=[];window.applied=[];const record={id:'record-id',version:'v1',classId:'2',date:'2026-09-03',name:'가상학생',phoneLast4:'1234',teacher:'최유정',source:'live',readState:'complete',schedule:'09:00 ~ 18:00',entry:'09:00:00',exit:'18:00:00',entryMemo:'기존 사유',exitMemo:'',outings:[]};const controller={refresh:async()=>{},state:()=>({records:[record],jobs:window.jobs||[],capabilities:['approved-requests-v1']}),api:async(path,input)=>{window.applied.push({path,input});if(window.failApply)throw Error('연결 끊김 시험');if(path==='apply')window.jobs=[{id:input.approvalId,status:'verified'}];if(path==='reconcile')window.rows.find(r=>r.id===input.approvalId).status='verified';return{};}};await mountCheckHereRequests(document.querySelector('#host'),{db:{},user:{email:'${email}',getIdTokenResult:async()=>({claims:{firebase:{sign_in_provider:'google.com'}}}),getIdToken:async()=>'fixture-token'},classes:[{id:'2'}],admin:${role!=='teacher'},controller:()=>controller});</script>`});
      ...
      if(role==='approver'){
        ...
        await page.evaluate(()=>{window.failApply=false;});await page.getByRole('button',{name:'승인된 변경 반영',exact:true}).click();await page.getByText('체크히어 자동 입력·저장 확인 완료',{exact:true}).waitFor();assert.equal(await page.evaluate(()=>window.rows[0].status),'verified');assert.equal(await page.evaluate(()=>window.applied.filter(c=>c.path==='apply').length),2,'failed dispatch then one explicit retry');assert.equal(await page.evaluate(()=>window.applied.filter(c=>c.path==='reconcile').length),1);
      }
      if(role==='teacher'){
