...
export async function mountAttendanceEvidence(host,{db,user,classes,teacherClass=null}){
 ...
 const active=()=>!disposed&&host.isConnected&&root.querySelector('#evidence')===content;
 const current=()=>Object.fromEntries([...selected].map(cid=>[cid,data[cid]||null]));
 function render(){if(!active())return;const report=evidenceReport(current(),{from,to,status}),message=evidenceMessage(report,{from,to});
  ...
  if($('#evSync'))$('#evSync').onclick=()=>void run(async()=>{for(const cid of selected){if(!active())return;notice=`${cid}반 동기화 중…`;render();const result=await syncAttendanceSummary(db,user,cid);if(!active())return;data[cid]=result;loaded.add(cid);}notice='선택한 반의 최신 출결·서류를 반영했습니다.';});
 ...
 async function read(force=false){await run(async()=>{for(const cid of selected){if(!active())return;if(force||!loaded.has(cid)){const result=await readAttendanceSummary(db,cid,{user,fresh:force});if(!active())return;data[cid]=result;loaded.add(cid);}}notice='저장된 서류 상태를 읽었습니다. 최신 정보는 마지막 출결 동기화 기준입니다.';});}
