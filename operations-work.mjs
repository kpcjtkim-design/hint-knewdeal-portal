...
export async function mountOperationsWork(host,{db,user,classes,mode='work',canApprove=false,onOpen=()=>{}}){
 ...
 function render(){
  ...
  $('#summary').innerHTML=`<div class="counts"><div>출결 확인<strong>${count('attendance')}건</strong><small>반별 기준 교육일</small></div><div>서류 미제출·반려<strong>${count('evidence')}건</strong><small>선택 기간 누적</small></div><div>설문 미응답<strong>${count('surveyItems')}건</strong><small>최신 기준과 일치하는 저장본</small></div><div>체크히어 대기 / 실패<strong>${count('pending')} / ${count('failed')}건</strong><small>재처리로 완료된 원본 제외</small></div></div>${unloaded?`<p class="warn">${unloaded}개 반 불러오는 중 · 아직 집계되지 않았습니다.</p>`:''}${reports.some(r=>r.notes.length)?'<p class="warn">미확인·갱신 필요 항목은 위 숫자에 포함되지 않을 수 있습니다. 아래 반별 확인사항을 함께 확인해 주세요.</p>':''}`;
  $('#details').innerHTML=reports.map(r=>`<details class="class-card"><summary><b>${r.classId}반</b><span>출결 ${r.attendance.length} · 서류 ${r.evidence.length} · 설문 ${r.surveyItems.length} · 요청 ${r.pending.length} · 실패 ${r.failed.length}</span><em class="${r.notes.length?'warn':''}">${r.notes.length?'자료 확인 필요':'저장 자료 집계'}</em></summary><p class="source">출결 확인 교육일 ${r.target||'미확인'} · 출결 동기화 ${esc(stamp(r.syncedAt))} · 서류·설문·요청 ${r.from>r.through?'선택 기간 시작 전':r.from+' ~ '+r.through}</p>${r.notes.length?`<p class="warn">${r.notes.map(esc).join(' · ')}</p>`:''}<div class="actions"><button data-open="attendanceOverview" data-cid="${r.classId}" data-date="${r.target}">출결대조 열기</button><button data-open="surveys" data-cid="${r.classId}">만족도조사 열기</button>${canApprove?'<button data-open="checkhere">체크히어 요청 처리</button>':''}</div><div data-items="${r.classId}"></div>${r.surveyUnchecked.length?`<p class="warn">설문 동기화 확인: ${r.surveyUnchecked.map(e=>esc(e.title)+' ('+esc(e.label)+')').join(' · ')}</p>`:''}${r.pending.length||r.failed.length?`<p>본부 처리: 승인·반영 대기 ${r.pending.length}건 / 실패 건 다시 처리 ${r.failed.length}건</p>`:''}</details>`).join('')||'<p>반을 선택해 주세요.</p>';
