...
export function createProposalReview({db,user,root,getContext,render,showErr,hasUnsavedReason,timeouts={}}){
 ...
 function html(s,column){
  ...
  const label=v.receipt?(v.receipt.state==='retry'?'동일 요청 번호로 재시도 가능':'접수 확인 필요'):active?PROPOSAL_STATUS[v.request.status]:disabled.startsWith('반영 확인 완료')?'반영 확인 완료':v.stale?'재검토 필요':v.request?.status==='withdrawn'?'취소 · 다시 요청 가능':changed?'변경 필요':'현재 기록과 일치';
