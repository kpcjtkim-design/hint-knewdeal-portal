...
export function openRawDownload(dialog,{kind,classes,selected,user,db,reader,catalog,signal}){
 return new Promise(resolve=>{
  const survey=kind==='survey';let closed=false,controller=null,tables=null,failed=0;
  ...
  const save=async()=>{lock(true);try{const XLSX=await loadExportXlsx();if(closed)return;XLSX.writeFile(rawWorkbook(XLSX,tables),`HINT_${survey?'만족도_원본한판':'출결_RAW'}_${koreaToday()}${failed?'_부분자료':''}.xlsx`,{compression:true});status.textContent='다운로드 완료. 수집내역에서 기간·실패·확인 필요 내역을 확인할 수 있습니다.';}catch(e){if(!closed)status.textContent=e.message;}finally{if(!closed)lock(false);}};
  $('#dlSave').onclick=save;
  $('#dlStart').onclick=async()=>{
   controller=new AbortController();const abort=controller.signal;tables=null;failed=0;lock(true);$('#dlSave').style.display='none';$('#dlClose').textContent='수집 취소';
   try{
    ...
    if(survey){
     ...
     const scope=classId==='all'?classes:classes.filter(c=>String(c.id)===classId);
     for(let i=0;i<scope.length;i++){
      abort.throwIfAborted();const c=scope[i];status.textContent=`${c.id}반 원본 읽는 중 · ${i+1}/${scope.length}`;
      try{let data;for(let attempt=0;attempt<3;attempt++){try{data=await readJson('/api/attendance-reader',{classId:String(c.id),idToken:await user.getIdToken(),allowCache:true},{signal:abort});break;}catch(e){abort.throwIfAborted();if(e.retryable===false||attempt===2)throw e;await pauseRead(5000*(attempt+1),abort);}}abort.throwIfAborted();const {readExportAttendance}=await import('./raw-attendance-store.mjs');const stored=await readExportAttendance(db,c.id,from,to,abort);abort.throwIfAborted();tables.push(...attendanceTables(data,c.id,from,to,stored));audit.push([c.id+'반','수집 완료','포털 DB 저장 구분 우선 · 시트 원본 비교표 포함']);}
      catch(e){abort.throwIfAborted();failed++;audit.push([c.id+'반','수집 실패',e.message]);}
