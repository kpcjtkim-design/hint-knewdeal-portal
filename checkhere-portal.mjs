...
export async function mountCheckHerePortal(host,{db,user,classes,showRequests=false}){
  ...
  const saveSnapshots=createSnapshotSaver({
    async read(classId,id){
      const snap=await within(getDocFromServer(doc(db,'classes',classId,'checkhereSnapshots',id)),20000,'DB 저장 확인이 지연됩니다. 다시 시도해 주세요.');
      if(snap.metadata?.fromCache||snap.metadata?.hasPendingWrites)throw Error('서버 저장 완료를 아직 확인하지 못했습니다.');
      return snap.exists()?snap.data():null;
    },
    async write(classId,id,data){await within(setDoc(doc(db,'classes',classId,'checkhereSnapshots',id),{...data,createdAt:serverTimestamp(),createdBy:user.email}),30000,'DB 저장 응답이 지연됩니다. 서버 기록을 다시 확인합니다.');},
  });
