...
let token='',email='',selectedClass='1',dates=[],students=[],reasonCells={},currentLabel='',editRows=[],dirty=false,currentReview=null;
...
async function mountReviewTools(){const host=$('#adminReviewToolsHost'),user=auth.currentUser;if(!host||!user)return;await mountAdminReviewTools(host,{auth,db,user,getState:()=>({selectedClass,currentLabel,dates,students,reasonCells}),parseReasonFor,standardizeReason,normalizeAttendanceStatus})}
