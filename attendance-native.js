...
function syncOverviewDocumentState(name,date,checked){root.querySelectorAll('.ov-doc-state').forEach(el=>{if(el.dataset.ovName===String(name)&&el.dataset.ovDate===String(date)){el.classList.toggle('done',checked);el.classList.toggle('pending',!checked);el.textContent=checked?'✓ 서류 확인':'서류 미확인'}})}
...
function setDirty(v=true){dirty=v;const b=$('#saveReviewBtn'),r=$('#resetReviewBtn');if(b)b.disabled=!dirty;if(r)r.disabled=!(currentReview?.overrides?.length||currentReview?.documentVerified?.length);const t=$('#dirtyText');if(t)t.textContent=dirty?'저장하지 않은 Firebase 수정 있음':'Firebase 수정 없음'}
...
function bindEditors(){root.querySelectorAll('.edit-status').forEach(el=>el.onchange=()=>{const x=editRows[+el.dataset.i];x.status=el.value;if(x.status==='출석'&&x.reason===x.baseReason)x.reason='';const w=root.querySelector(`[data-doc-wrap="${el.dataset.i}"]`);if(w)w.classList.toggle('hidden',x.status!=='인정출석');setDirty();recalcSummary()});root.querySelectorAll('.edit-reason').forEach(el=>el.oninput=()=>{editRows[+el.dataset.i].reason=el.value;setDirty()});root.querySelectorAll('.doc-verify-check').forEach(el=>el.onchange=()=>saveDocumentVerification(+el.dataset.i,el.checked,el).catch(showError));root.querySelectorAll('[data-history-i]').forEach(el=>el.onclick=()=>openStudentHistory(+el.dataset.historyI))}
...
async function renderDate(label){
  ...
  $('#rows').innerHTML=students.map((s,i)=>{
    ...
    return `<div class="student ${flags.length?'has-flags':''}"><div class="name"><button type="button" class="student-name-btn" data-history-i="${i}">${esc(s.name)}</button>${ov?'<span class="firebase-mark">Firebase 수정</span>':''}</div><div><select class="edit-status" data-i="${i}">${options.map(x=>`<option value="${esc(x)}" ${status===x?'selected':''}>${esc(x)}</option>`).join('')}</select></div><div><input class="edit-reason" data-i="${i}" value="${esc(reason)}" placeholder="사유 없음 / 직접 수정 가능"><div class="save-meta">시트 기준: ${esc(baseStatus)}${baseReason?' · '+esc(baseReason):''}${parsed.method&&parsed.method!=='none'?' · '+esc(parsed.method):''}</div>${docHtml}${docHtml}${flagHtml}</div></div>`
