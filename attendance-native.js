...
async function saveDocumentReview(i,state,el){const x=editRows[i];if(!x||!currentLabel)return;const next=documentReviewMap(currentReview);const value=['확인','보완필요','미제출'].includes(state)?state:'미제출';if(value==='미제출')next.delete(x.name);else next.set(x.name,value);const stateEl=root.querySelector(`[data-doc-state=\"${i}\"]`);if(el)el.disabled=true;if(stateEl)stateEl.textContent='저장 중…';try{const documentReview=Object.fromEntries([...next].sort((a,b)=>a[0].localeCompare(b[0],'ko')));const documentVerified=Object.entries(documentReview).filter(([,v])=>v==='확인').map(([k])=>k);await setDoc(doc(db,'settings',reviewDocId(currentLabel)),{type:'ATTENDANCE_REVIEW_LIVE',source:'운영총괄_ORIGINAL_READ_ONLY',classId:selectedClass,date:currentLabel,documentReview,documentVerified,documentVerifier:email,documentVerifiedAt:serverTimestamp()},{merge:true});currentReview={...(currentReview||{}),documentReview,documentVerified};if(stateEl)stateEl.textContent=value==='확인'?'확인 저장됨':value==='보완필요'?'보완필요 저장됨':'미제출 저장됨';syncOverviewDocumentState(x.name,currentLabel,value);setDirty(dirty)}catch(e){if(stateEl)stateEl.textContent='저장 실패';throw e}finally{if(el)el.disabled=false}}
function syncOverviewDocumentState(name,date,state){root.querySelectorAll('.ov-doc-state').forEach(el=>{if(el.dataset.ovName===String(name)&&el.dataset.ovDate===String(date)){el.classList.toggle('done',state==='확인');el.classList.toggle('rejected',state==='보완필요');el.classList.toggle('pending',state==='미제출');el.textContent=state==='확인'?'✓ 확인':state==='보완필요'?'보완필요':'미제출'}})}
async function loadAttendanceColors(){
...
function overviewItemHtml(x,type){
  const ds=x.docState==='확인'?'done':x.docState==='보완필요'?'rejected':'pending';
  const doc=type==='recognized'?`<span class="ov-doc-state ${ds}" data-ov-name="${esc(x.name)}" data-ov-date="${esc(x.date)}">${x.docState==='확인'?'✓ 확인':x.docState==='보완필요'?'보완필요':'미제출'}</span>`:'';
...
async function renderDate(label){
  ...
  $('#rows').innerHTML=students.map((s,i)=>{
    ...
    const firebaseDocState=docReview.get(s.name)||'미제출';
