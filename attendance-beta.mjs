...
import {checkHerePhoneFor} from './checkhere-name-core.mjs';
...
export function createAttendanceBeta({db,user,root,state,render,showErr,reasonFor,colorState,onStatusSaved=()=>{}}){
  ...
  let identities=[],timetableEntries=[],metadata={},snapshots=[],snapshotError='',excursions=[],lectureEnds=[],excursionError='',working=false,loading=false,sequence=0;
  ...
  function displayStatus(s){if(statusPreview.has(key(s)))return statusPreview.get(key(s));const raw=String(s.all[state().date.idx]||'').trim();try{return deriveRecognized(raw,info(s),matchSnapshot(s,state().students,snapshots,identities).record,{excursion:excursions.length>0}).status;}catch{return raw||'해당없음';}}
  function displayEvidence(s){return evidenceStatus(rawColor(s),String(s.all[state().date.idx]||''),info(s),colorState);}
  const proposals=createProposalReview({db,user,root,render,showErr,hasUnsavedReason:()=>drafts.size>0,getContext:s=>({classId:state().classId,date:state().iso,name:s.name,phoneLast4:checkHerePhoneFor(s,identities)||'',status:displayStatus(s),reason:drafts.get(key(s))??currentReason(s),reasonUnsaved:drafts.has(key(s)),statusSaving:statusPreview.has(key(s)),teacher:teachers.get(state().classId,snapshots,matchSnapshot(s,state().students,snapshots,identities).record),raw:state().raw,excursion:excursions.length>0,record:matchSnapshot(s,state().students,snapshots,identities).record})});
  ...
  function controls(s){
    ...
    const derived=deriveRecognized(String(s.all[state().date.idx]||'').trim(),info(s),matchSnapshot(s,state().students,snapshots,identities).record,{excursion:excursions.length>0});
  ...
  function snapshotCells(s,memoButton=()=> ''){
    ...
    const {record:r,error}=matchSnapshot(s,state().students,snapshots,identities);
  ...
  return {timetableEntries:()=>timetableEntries,controls,snapshotCells,bind,displayStatus,displayEvidence,currentReason,draftReason:s=>drafts.get(key(s))??currentReason(s),snapshotFor:s=>matchSnapshot(s,state().students,snapshots,identities).record,
    ...
    async readRefresh(classId,date){const [meta,records,proposal,published,draft,names]=await Promise.all([getDoc(doc(db,'settings',`attendanceBeta_${classId}_${date}`)),loadCheckHereDay(db,classId,date),proposals.read(classId,date),getDoc(doc(db,'timetableBetaPublished',String(classId))),getDoc(doc(db,'timetableBetaDrafts',String(classId))),loadCheckHereIdentities(db,classId,state().students)].map(p=>within(p)));return {identities:names,metadata:meta.data()?.students||{},snapshots:records,proposal,entries:draft.data()?.entries||published.data()?.entries||[]};},
    applyRefresh(data,date){identities=data.identities;metadata=data.metadata;snapshots=data.snapshots;snapshotError='';excursionError='';excursions=excursionFor(data.entries,date);lectureEnds=lectureEndsOn(data.entries,date);proposals.applyRead(data.proposal);headerState();void refreshTeacher(state().classId);},
    ...
    async load(classId,date){void loadSurveyLinks().then(()=>headerState());const n=++sequence;loading=true;drafts.clear();identities=[];timetableEntries=[];metadata={};snapshots=[];snapshotError='';excursions=[];lectureEnds=[];excursionError='';headerState();const results=await Promise.allSettled([getDoc(doc(db,'settings',`attendanceBeta_${classId}_${date}`)),loadCheckHereDay(db,classId,date),proposals.load(classId,date),getDoc(doc(db,'timetableBetaPublished',String(classId))),getDoc(doc(db,'timetableBetaDrafts',String(classId))),loadCheckHereIdentities(db,classId,state().students)].map(p=>within(p)));if(n!==sequence)return;loading=false;if(results[0].status==='fulfilled')metadata=results[0].value.data()?.students||{};else showErr(new Error('포털 세부 구분을 불러오지 못했습니다. '+results[0].reason.message));if(results[5].status==='fulfilled')identities=results[5].value;else showErr(new Error('동명이인 구분 정보를 읽지 못했습니다. 다시 읽기를 눌러 주세요.'));if(results[1].status==='fulfilled')snapshots=results[1].value;else snapshotError='체크히어 저장본 조회 실패 · 다시 읽기 필요';if(results[3].status==='fulfilled'&&results[4].status==='fulfilled'){const published=results[3].value.data(),draft=results[4].value.data();const entries=draft?.entries||published?.entries||[];timetableEntries=entries;excursions=excursionFor(entries,date);lectureEnds=lectureEndsOn(entries,date);}else excursionError='견학일 여부 확인 실패 · 시간표를 확인해 주세요.';headerState();void refreshTeacher(classId,n);},
  };
}
