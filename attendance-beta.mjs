...
export function createAttendanceBeta({db,user,root,state,render,showErr,reasonFor,colorState,onStatusSaved=()=>{}}){
  ...
  let timetableEntries=[],metadata={},snapshots=[],snapshotError='',excursions=[],lectureEnds=[],excursionError='',working=false,loading=false,sequence=0;
  ...
  return {timetableEntries:()=>timetableEntries,controls,snapshotCells,bind,displayStatus,displayEvidence,currentReason,draftReason:s=>drafts.get(key(s))??currentReason(s),snapshotFor:s=>matchSnapshot(s,state().students,snapshots).record,
