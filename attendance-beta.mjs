...
export function createAttendanceBeta({db,user,root,state,render,showErr,reasonFor,colorState}){
  ...
  function bind(){
    ...
    root.querySelectorAll('.beta-reason').forEach(el=>el.oninput=()=>{const s=student(el.dataset.student);if(el.value===currentReason(s))drafts.delete(key(s));else drafts.set(key(s),el.value);proposals.refresh(state().students);headerState();});headerState();
  }
  ...
  return {controls,snapshotCells,bind,displayStatus,displayEvidence,currentReason,draftReason:s=>drafts.get(key(s))??currentReason(s),snapshotFor:s=>matchSnapshot(s,state().students,snapshots).record,
    ...
    async readRefresh(classId,date){const [meta,records,proposal,published,draft]=await Promise.all([getDoc(doc(db,'settings',`attendanceBeta_${classId}_${date}`)),loadCheckHereDay(db,classId,date),proposals.read(classId,date),getDoc(doc(db,'timetableBetaPublished',String(classId))),getDoc(doc(db,'timetableBetaDrafts',String(classId)))]);return {metadata:meta.data()?.students||{},snapshots:records,proposal,entries:draft.data()?.entries||published.data()?.entries||[]};},
