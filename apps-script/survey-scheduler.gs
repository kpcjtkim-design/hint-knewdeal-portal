...
function installSurveyScheduler() {
  ...
  console.log('한국시간 09~18시 정각 및 18:10/18:20/18:30 예약 활성화. 실행기는 매분 예정 작업을 확인합니다.');
  surveySchedulerTick();
}
...
function surveySchedulerStatus() {
  const props=PropertiesService.getScriptProperties();
  console.log('예약 사용: '+props.getProperty('SURVEY_ENABLED'));
  console.log(props.getProperty('SURVEY_PENDING') || props.getProperty('SURVEY_LAST_REPORT') || '아직 실행 결과가 없습니다.');
}
function surveySchedulerTick() {
  ...
  try {
    const slot=surveyDueSlot_(new Date());if(!slot)return;
    const saved=props.getProperty('SURVEY_PENDING');
    ...
    while(job.next<=17 && Date.now()-started<180000){
      ...
      try {const result=surveyCall_({action:'class',slot:job.slot,classId:cid}).result;result.failures=(result.failures||[]).slice(0,2);job.results.push(result);job.next++;console.log(cid+'반: 설문 '+result.done+'건, 변경 '+result.changed+'건, 실패 '+result.failed+'건');}
    ...
    if(job.next>17){const result=surveyCall_({action:'finish',slot:job.slot,results:job.results});props.setProperty('SURVEY_LAST_SLOT',job.slot);props.setProperty('SURVEY_LAST_REPORT',JSON.stringify({slot:job.slot,state:result.state,results:job.results}));props.deleteProperty('SURVEY_PENDING');console.log('동기화 '+result.state+' · '+job.slot+' · '+job.results.length+'개 반 · 읽기 '+job.results.reduce((n,r)=>n+(r.reads||0),0)+' · 쓰기 '+job.results.reduce((n,r)=>n+(r.writes||0),0));}
